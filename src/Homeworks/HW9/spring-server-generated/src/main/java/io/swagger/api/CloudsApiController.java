...
public class CloudsApiController implements CloudsApi {
    ...
    public ResponseEntity<Void> cancelCloudById(@Parameter(in = ParameterIn.PATH, description = "Индентификатор заказа облака", required=true, schema=@Schema()) @PathVariable("cloud_id") String cloudId) {
    ...
    public ResponseEntity<Cloud> getCloudById(@Parameter(in = ParameterIn.PATH, description = "Индентификатор заказа облака", required=true, schema=@Schema()) @PathVariable("cloud_id") String cloudId) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
